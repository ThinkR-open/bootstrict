...
test_that("as_msg_list keeps length-1 vectors as arrays", {
  expect_null(bootstrict:::as_msg_list(
    NULL
  ))
  expect_equal(
    bootstrict:::as_msg_list(
      "a"
    ),
    list(
      "a"
...
test_that("toast_placement_class maps every placement keyword", {
  expect_equal(
    bootstrict:::toast_placement_class(
      "top-start"
    ),
    "top-0 start-0"
  )
...
test_that("bs_color_input builds a colour control; update_* route through sendInputMessage", {
  ...
  expect_error(update_bs_range(
    "vol",
    1,
    session = NULL
  ))
  expect_error(update_bs_color(
    "col",
    "#fff",
    session = NULL
  ))
...
test_that("floating label reuses the control's own label text", {
  out <- render(bs_floating_label(bs_text_input(
    "email",
    "Email address"
  )))
  expect_match(
...
test_that("find_label_text returns NULL when no label exists in the tree", {
  expect_null(
    bootstrict:::find_label_text(htmltools::div(htmltools::span(
      "hi"
    )))
  )
})
...
test_that("alert heading and link render their classes", {
  expect_match(
    render(bs_alert_heading(
      "Title",
      level = 3
    )),
    "<h3[^>]*alert-heading"
  )
  out <- render(bs_alert_link(
    "more",
    href = "/x"
  ))
  expect_match(
...
test_that("card footer / subtitle / link / img / overlay / group render", {
  expect_match(
    render(bs_card_footer(
      "f"
    )),
    "card-footer"
  )
  expect_match(
    render(bs_card_subtitle(
      "s"
    )),
    "card-subtitle"
  )
  ...
  expect_match(
    render(bs_card_img_overlay(
      "x"
    )),
    "card-img-overlay"
  )
  expect_match(
    render(bs_card_group(bs_card(bs_card_body(
      "x"
    )))),
    "card-group"
  )
...
test_that("bs_table builds head + body from a data frame, formatting NA cells", {
  ...
  built <- bootstrict:::bs_table_from_data(
    noname
  )
  expect_match(
    render(
...
test_that("page constructors attach the bootstrict dependency", {
  ...
  expect_true(has_dep(bs_page(
    title = "t",
    bs_container(
      "hi"
    )
  )))
  expect_true(has_dep(bs_page_fluid(bs_container(
    "hi"
  ))))
  expect_true(has_dep(bs_page_fillable(bs_container(
    "hi"
  ))))
})
...
test_that("build_bootstrict_dep tolerates a missing stylesheet", {
  ...
  dep <- bootstrict:::build_bootstrict_dep(
    assets
  )
  expect_s3_class(
    dep,
...
test_that("internal helpers cover their edge branches", {
  expect_true(bootstrict:::is_empty(
    NULL
  ))
  expect_true(bootstrict:::is_empty(character(
    0
  )))
  expect_false(bootstrict:::is_empty(
    "x"
  ))

  expect_error(
    bootstrict:::match_arg(
      NULL,
      c(
        "a",
        "b"
  ...
  expect_null(bootstrict:::bs_bool_attr(
    NULL
  ))
  expect_equal(
    bootstrict:::bs_bool_attr(
      TRUE
    ),
    "true"
  )
  ...
  expect_false(bootstrict:::has_class(
    htmltools::div(),
    "x"
  ))
  expect_false(bootstrict:::has_class(
  ...
  tree <- htmltools::div(htmltools::span(
    class = "needle",
    "x"
  ))
...
test_that("bs_alert reports its state only when given an id", {
  expect_match(
    render(bs_alert(
      "Saved",
      id = "a",
    ...
    "id=\"a\" data-bootstrict=\"alert\""
  )
  # No id, no binding: a decorative alert stays inert.
  expect_no_match(
    render(bs_alert(
      "Plain"
    )),
...
test_that("close_bs_alert dispatches a namespaced message", {
  s <- mock_session()
  close_bs_alert(
    "a",
    session = s
