...
test_that("text-like control markup is stable", {
  ...
  expect_snapshot(snap(bs_textarea_input("bio", "Bio", rows = 3)))
  expect_snapshot(snap(bs_numeric_input("n", "N", value = 1, min = 0, max = 10)))
  expect_snapshot(snap(bs_password_input("pw", "Password", size = "lg")))
  expect_snapshot(snap(bs_select_input("s", "Select", c("a", "b"))))
})

test_that("check-like control markup is stable", {
  expect_snapshot(snap(bs_checkbox_input("cb", "Agree")))
  expect_snapshot(snap(bs_switch_input("sw", "Enable")))
  expect_snapshot(snap(bs_radio_input("r", "Size", c("S", "M"), inline = TRUE)))
  expect_snapshot(snap(bs_checkbox_group_input("cg", "Pick", c("a", "b"))))
  expect_snapshot(snap(bs_checkbox_input("cbr", "Reverse", reverse = TRUE)))
})

test_that("native control markup is stable", {
  expect_snapshot(snap(bs_range_input("rng", "Range", value = 5, min = 0, max = 10)))
  expect_snapshot(snap(bs_color_input("col", "Colour", value = "#ff6600")))
})
...
test_that("form layout markup is stable", {
  ...
  expect_snapshot(snap(bs_input_group(
    bs_input_group_text(bs_checkbox_input("cb", NULL)),
    bs_text_input("u", NULL)
  )))
  expect_snapshot(snap(bs_floating_label(bs_text_input("e", "Email"))))
  expect_snapshot(snap(bs_form(
    bs_form_label("e", "Email"),
    bs_text_input("e", NULL),
    bs_form_text("We never share it.")
  )))
  expect_snapshot(snap(bs_feedback(
    bs_text_input("user", "User"),
    invalid = "Taken.",
    state = "invalid"
  )))
  expect_snapshot(snap(bs_valid_feedback("Looks good.")))
  expect_snapshot(snap(bs_invalid_feedback("Please provide a value.")))
})
...
test_that("the shiny-inherited controls are recorded as they stand", {
  ...
  expect_snapshot(snap(bs_file_input("f", "Upload")))
  expect_snapshot(snap(bs_date_input("d", "Date")))
  expect_snapshot(snap(bs_date_range_input("dr", "Range")))
})
